  ...
  if (is(chunk, Column)) {
    ...
    result.sql +=
      chunk.table[IsAlias] || schemaName === undefined
        ...
        : config.escapeName(schemaName) +
          "." +
          config.escapeName(chunk.table[Table.Symbol.Name]) +
          "." +
          config.escapeName(columnName);
    return;
  }
...
function mapInlineParam(chunk: unknown, config: BuildQueryConfig): string {
  if (chunk === null) {
    return "null";
  }
  ...
  if (typeof chunk === "object") {
    ...
    return config.escapeString(
      mappedValueAsString === "[object Object]" ? JSON.stringify(chunk) : mappedValueAsString,
    );
  }
...
export class SQL<T = unknown> implements SQLWrapper {
  ...
  buildQueryFromSourceParams(chunks: SQLChunk[], _config: BuildQueryConfig): Query {
    ...
    mergeQueryWithChunks(
      chunks,
      config as BuildQueryConfig &
        Required<Pick<BuildQueryConfig, "inlineParams" | "paramStartIndex">>,
      result,
    );
    return result;
  }
...
> implements SQLWrapper {
  ...
  declare readonly $inferSelect: InferSelectViewModel<
